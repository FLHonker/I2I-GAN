...
#         g_model = None
#         for _ in range(2):
#             g_model = [nn.ConvTranspose2d(in_features, out_features, 3, stride=2, padding=1, output_padding=1),
#                       nn.InstanceNorm2d(out_features),
#                       nn.ReLU(inplace=True)]
#             in_features = out_features
#             out_features = in_features // 2
...
#
#         self.model = nn.Sequential(*model)
#         self.g_model = nn.Sequential(*g_model)
#
#     def forward(self, x, gen=True):
#         feature_out = self.model(x)
#         if not gen:
#             return feature_out
#         gen = self.g_model(feature_out)
#         return gen
#
#
...
#
#         model = [nn.Conv2d(in_features, 256, 4, stride=2, padding=1),
...
class PeerNet(nn.Module):
    def __init__(self, input_dim, num_filter=32):
        super(PeerNet, self).__init__()
        ...
        for i in range(6):
            self.resnet_blocks.append(ResBlock(num_filter * 4))
    ...
    def forward(self, x):
        ...
        feat = self.conv3(enc2)
        feat = self.resnet_blocks(feat)
        feat = self.comv4(feat)
        return feat  # torch.Size([1, 128, 64, 64])


class PGNet(nn.Module):
    def __init__(self, output_dim, num_filter=32, num_resnet=4):
        super(PGNet, self).__init__()
        ...
        for i in range(num_resnet):
            self.resnet_blocks.append(ResBlock(num_filter * 8))
        self.resnet_blocks = torch.nn.Sequential(*self.resnet_blocks)
        # Decoder
    ...
    def forward(self, feat):
        # Resnet blocks
        res = self.resnet_blocks(feat)
        # Decoder
        dec1 = self.deconv1(res)
        dec2 = self.deconv2(dec1)
        out = self.deconv3(self.pad(dec2))
        return out  # torch.Size([1, 3, 256, 256])


class PDNet(nn.Module):
    def __init__(self, input_dim=512):   # torch.Size([1, 128*2, 64, 64])
        super(PDNet, self).__init__()

        conv1 = ConvBlock(input_dim, 256, kernel_size=4, stride=1, padding=1, activation='lrelu')
        conv2 = ConvBlock(256, 128, kernel_size=4, stride=1, padding=1, activation='lrelu')
        conv3 = ConvBlock(128, 1, kernel_size=4, stride=1, padding=1, activation='no_act', batch_norm=False)
...
class P2Generator(nn.Module):
    def __init__(self, input_dim, output_dim=3, num_filter=32, num_resnet=6):
        super(P2Generator, self).__init__()
...
class P2Discriminator(nn.Module):
    def __init__(self, input_dim, output_dim=1, num_filter=64):
        super(P2Discriminator, self).__init__()
